...
#omg1 = Gaeilge['GAO']
#Below is an approach to work around this. 
#'GAO' is not one of the keys in Gaeilge. The get method looks for 'GAO' and if it cannot find it returns 'acronym missing')
omg2 = Gaeilge.get('GAO','acronym missing') #this method is preferred as it provides an option for an error message. 
...
#This approach removes it completely like deleting it. However it has the advantage that it returns the deleted element if needed. 
print(Gaeilge)
Gaeilge.pop('OMD')
print(Gaeilge)
#the other way to delete an element is to use del
#del d["LOL"]
#print(d)
...
Gaeilge.clear()
print(Gaeilge)
#This can also be achieved by Gaeilge={} #similar to creating an empty dict